...
This folder holds the starter program for **Session 2: Maze Runner**.
...
*(This reuses your calibrated `wheel_diameter` / `axle_track` from Session 1 — if
turns are off, re-check `axle_track` before blaming the maze.)*

## Quick troubleshooting

* **Robot bumps a wall?** That's sensor data — find the step in your sequence that
  went wrong and fix *that* one move, then run again.
