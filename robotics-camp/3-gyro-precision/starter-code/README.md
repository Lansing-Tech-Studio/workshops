...
This folder holds the starter program for **Session 3: Gyro Precision**.
...
## Reuse your setup from Session 1

The top of `gyro_drive.py` is the **same** hub/motor/DriveBase setup you calibrated in
Session 1 — use **your** calibrated `wheel_diameter`, `axle_track`, and verified ports
...
1. **Off vs. on:** Run the same square (or maze path) with the gyro **off**, mark where
   it ends, then add `use_gyro(True)` and run the *exact same code* again. Chart the
