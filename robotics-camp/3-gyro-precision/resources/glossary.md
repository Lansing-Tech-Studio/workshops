...
is *turning*. It lets Techster feel which way it's facing instead of just counting
wheel-spins.

**gyroscope:** The full name for the gyro. It measures rotation, which the robot uses to
make turns and straights more accurate.

**IMU:** Stands for *inertial measurement unit* — the chip inside the hub that holds the
...
turns *more accurate* — they end up closer to the angle you typed.
...
**calibration:** Measuring what the robot *actually* does and adjusting the numbers until
...
Session 1). It's the thing you call `use_gyro()`, `straight()`, and `turn()` on.
